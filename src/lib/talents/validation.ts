...
export function parseTalentInput(formData: FormData): TalentInput {
  return {
    ...
    subcategorias: list(formData, 'subcategorias'),
...
export function validateTalentInput(input: TalentInput): string | null {
  if (!slugPattern.test(input.slug) || input.slug.length > 120) {
    return 'Use um slug com até 120 caracteres, apenas letras minúsculas, números e hífens.';
  }
  ...
  if (input.categoria.length < 2 || input.categoria.length > 100) return 'A categoria deve ter entre 2 e 100 caracteres.';
  if (input.subcategorias.length > 20 || input.subcategorias.some((item) => item.length > 100)) {
    return 'Informe no máximo 20 subcategorias, com até 100 caracteres cada.';
  }
  ...
  if (input.biografia.length < 20 || input.biografia.length > 5000) {
    return 'A biografia deve ter entre 20 e 5.000 caracteres.';
  }
  if (input.habilidades.length > 30 || input.habilidades.some((item) => item.length > 120)) {
    return 'Informe no máximo 30 habilidades, com até 120 caracteres cada.';
  }
  ...
  if (!Number.isInteger(input.ordem) || input.ordem < 0 || input.ordem > 9999) {
    return 'A ordem deve ser um número inteiro entre 0 e 9.999.';
  }
  return null;
}
