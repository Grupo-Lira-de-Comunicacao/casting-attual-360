...
export default async function HomePage() {
  ...
  return (
    <SiteShell>
      ...
      <section className="py-20"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-[.28em] text-teal">Casting em destaque</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">Talentos que movem histórias.</h2><p className="mt-4 text-base text-slate-400">{usingFallback ? 'O catálogo está temporariamente indisponível. Por segurança, apenas perfis reais são exibidos no ambiente oficial.' : featuredTalents.length > 0 ? 'Perfis selecionados pela curadoria do Casting Attual 360.' : 'Novos perfis serão publicados após a curadoria e validação.'}</p></div><Link href="/talentos" className="font-bold text-teal">Explorar catálogo <span aria-hidden="true">→</span></Link></div>{featuredTalents.length > 0 && <div className="mt-10 grid gap-6 md:grid-cols-3">{featuredTalents.map((talent) => <TalentCard key={talent.slug} talent={talent} />)}</div>}</section>
