...
type PageProps = {
  ...
  searchParams?: Promise<{ created?: string; integration?: string }>;
};
  ...
  return (
    <SiteShell>
      <div className="space-y-7">
        <div><Link href="/admin/producoes" className="font-bold text-teal">← Voltar às produções</Link></div>
        ...
        {query.integration === 'queued' && <p className="rounded-2xl border border-blue/20 bg-blue/5 px-5 py-4 font-semibold text-navy">Evento <code>casting.production.created</code> registrado para o ATTUAL ONE.</p>}
        {query.integration === 'warning' && <p className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 font-semibold text-amber-900">A produção foi criada, mas o evento de integração não pôde ser enfileirado. A produção não foi desfeita.</p>}
        ...
        <section className="rounded-[28px] border border-dashed border-blue/30 bg-blue/5 p-7">
          ...
          <p className="mt-3 max-w-3xl text-slate-600">Esta produção agora tem identidade própria. A próxima missão ligará necessidades de elenco, requisitos, shortlist e convites Telegram a este ID.</p>
        </section>
      </div>
    </SiteShell>
  );
